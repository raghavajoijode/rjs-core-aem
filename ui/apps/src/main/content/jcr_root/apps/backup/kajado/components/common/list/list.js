                ...
                .then(function (pageContainingItem) {
                    ...
                    ResourceUtils.getPageProperties(pageContainingItem)
                        .then(function (pageProps) {
                            ...
                            pageContainingItem.thumbnailUrl = pageContainingItem.path + "/jcr:content.thumbnail.48.48.jpg?ch_ck=" + (pageProps["cq:lastModified"] || pageProps["jcr:lastModified"]).getTimeInMillis();
            ...
            .then(function (startResource) {
                ResourceUtils.getContainingPage(startResource).then(function (startPage) {
                    ...
                    startPage.getChildren().then(function (childList) {
                        // remove the jcr:content + rep:policy children, if they exist
                        for(var childIdx = 0 ; childIdx < childList.length ; childIdx++) {
                            var child = childList[childIdx];
                            if (child.name == "jcr:content" || child.name == "rep:policy") {
                                log.debug("Omitting resource " + childList[childIdx].path + " from list component.");
