var platform = {
	...
	addBodyClass: function(d){
		qPage.onReady(function(){
			if(document.body.className!=""&&document.body.className.charAt(document.body.className.length-1)!=" "){document.body.className+=" ";}
			document.body.className+=d;
		});
	},
	isMobilePhone: function(d){
		if(window.devicePixelRatio && window.screen.width){
			var pixelRatio = (navigator.userAgent.match(/(iPad|iPhone|iPod)/g) ? 1 : window.devicePixelRatio );
			var screenWidth = (window.screen.width / pixelRatio);
			if(screenWidth <= 640 && this.device != false){
				return true;
			}
		}
		return false;
