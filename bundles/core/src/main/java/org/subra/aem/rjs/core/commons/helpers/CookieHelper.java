...
import org.subra.aem.rjs.core.commons.constants.CookieType;
import org.subra.aem.rjs.core.commons.utils.RJSCollectionUtils;
...
public class CookieHelper {
    ...
    public static String[] getAuthKeys(final HttpServletRequest request) {
        if (request != null && request.getCookies() != null)
            return RJSCollectionUtils.getStreamFromArray(request.getCookies()).filter(c -> StringUtils.equals(c.getName(), CookieType.AUTH_KEY.getName())).map(Cookie::getValue).toArray(String[]::new);
    ...
    public static void setAuthKey(final HttpServletResponse response, final String domain, final String value, final boolean createSecure) {
        setCookieOfType(response, CookieType.AUTH_KEY, value, domain, COOKIE_EXPIRATION, createSecure, createSecure);
    }

    public static void setAccountId(final HttpServletResponse response, final String value, final boolean createSecure) {
    ...
    public static String getCookieValue(final HttpServletRequest request, final String cookieName) {
        if (request != null && request.getCookies() != null) {
            final Cookie cookie = Arrays.stream(request.getCookies()).filter(c -> StringUtils.equals(c.getName(), cookieName)).findFirst().orElse(null);
            return cookie != null ? cookie.getValue() : null;
        }
        return null;
    }

    private static void setCookieOfType(final HttpServletResponse response, final CookieType cookieType, final String value, final String domain, final int expiry, final boolean secure, final boolean httpOnly) {
        setCookie(response, cookieType.getName(), value, domain, expiry, secure, httpOnly);
    }

    public static void setCookie(final HttpServletResponse response, final String cookieName, final String value, final String domain, final int expiry, final boolean secure, final boolean httpOnly) {
        if (value != null) {
            if (httpOnly) {
    ...
    protected static void createHttpOnlyCookie(final HttpServletResponse response, final String cookieName, final String domain, final String value, final int expiry, final boolean secure) {
    ...
    protected static String validateCookieValue(final String value) {
        String newVal = value;
        for (String cookieBadCharacter : COOKIE_BAD_CHARACTERS) {
            newVal = newVal.replaceAll(cookieBadCharacter, StringUtils.EMPTY);
        }
        return newVal;
    }
